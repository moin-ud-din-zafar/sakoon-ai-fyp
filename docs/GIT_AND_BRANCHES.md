...
| Branch | Remote | Use |
|--------|--------|-----|
| **`fullstack_Sakoon_AI_`** | `sakoon` (moin-ud-din-zafar/sakoon-ai-fyp) | **Team branch (force-updated)** — JWT + docs + payload guide |
| **`feature/jwt-email-password-auth`** | `origin` / `sakoon` | Same code, alternate branch name |
| `Backend_sakoon_Ai` | `origin` / `sakoon` | Older name; use `fullstack_Sakoon_AI_` |
